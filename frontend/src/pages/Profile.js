...
//   useEffect(() => {
//     // Ustawiamy username z userId jeśli user.username nie jest dostępny
//     const currentUserName = user?.username || userId;
...
//         // Jeśli brak tokenu, nie próbujemy pobierać danych autoryzowanych
//         if (!token) {
//           setLoading(false);
//           return;
//         }
...
//     fetchProfileData();
//   }, [userId, user]); // Zmiana zależności na user

//   const toggleSubscription = async () => {
//     if (!user) return; // Jeśli użytkownik nie jest zalogowany, nic nie rób

//     const token = Cookies.get(`auth_token_${user.username}`);
//     if (!token) return; // Jeśli brak tokenu, nie wykonuj żadnej akcji
...
//   const handleProfilePictureUpload = async (event) => {
//     if (!user) return; // Jeśli użytkownik nie jest zalogowany, nie wykonuj żadnej akcji

//     const file = event.target.files[0];
//     if (!file) return;

//     const token = Cookies.get(`auth_token_${user.username}`);
//     if (!token) return; // Jeśli brak tokenu, nie pozwalamy na zmianę zdjęcia
...
//             {/* Jeśli nie ma tokenu, przycisk subskrypcji będzie niewidoczny */}
//             {user && Cookies.get(`auth_token_${userId}`) && user.username !== userId && (
//               <button onClick={toggleSubscription}>
//                 {isSubscribed ? 'Usuń subskrypcję' : 'Subskrybuj'}
//               </button>
//             )}
...
//         setFollowersCount((prevCount) => prevCount - 1);
//       } else {
//         await axios.post('/followers/subscribe', { followingUsername: userName }, {
//           headers: { Authorization: `Bearer ${token}` },
//         });
//         setIsSubscribed(true);
//         setFollowersCount((prevCount) => prevCount + 1);
...
//                 <input
//                   type="text"
//                   placeholder="Nowe imię użytkownika"
//                   value={newUserName}
//                   onChange={(e) => setNewUserName(e.target.value)}
//                 />
//                 <button onClick={handleUsernameChange}>Zmień imię</button>
//               </div>
//             )}
